...
class _AppState extends State<App> {
  ...
  Future<void> openSms(String phone, String text) async {
    ...
    await launchUrl(Uri(scheme: 'sms', path: phone, queryParameters: text.trim().isEmpty ? null : {'body': text}), mode: LaunchMode.externalApplication);
  }

  Future<void> editContact([Contact? c]) async {
    final r = await showModalBottomSheet<Contact>(context: context, isScrollControlled: true, builder: (_) => ContactForm(c));
    if (r == null) return;
    setState(() {
  ...
  Future<void> editJob([Job? j, Contact? c]) async {
    final r = await showModalBottomSheet<Job>(context: context, isScrollControlled: true, builder: (_) => JobForm(contacts: contacts, job: j, contact: c));
    if (r == null) return;
    setState(() {
      final i = jobs.indexWhere((x) => x.id == r.id);
      if (i < 0) jobs.add(r); else jobs[i] = r;
    });
    await save();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: [
        ...
        ContactPage(contacts: contacts, onEdit: editContact, onDelete: (c) { setState(() => contacts.remove(c)); save(); }, onSms: (c) => openSms(c.phone, ''), onSchedule: (c) => editJob(null, c), onFav: toggleFavorite),
        SchedulePage(jobs: jobs, onEdit: editJob, onOpen: (j) => openSms(j.phone, j.text), onSent: (j) { setState(() => jobs[jobs.indexOf(j)] = j.copy(status: Status.sent)); save(); }, onCancel: (j) { setState(() => jobs[jobs.indexOf(j)] = j.copy(status: Status.cancelled)); save(); }),
      ][tab],
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.event_note), label: 'Scheduler'),
        ],
      ),
    );
  }
...
class _SmsPageState extends State<SmsPage> {
  ...
  @override
  Widget build(BuildContext c) => Page(title: 'SMS', children: [
    ...
    gap,
    TextField(controller: phone, enabled: id.isEmpty, decoration: const InputDecoration(labelText: 'Phone', border: OutlineInputBorder())),
    gap,
    TextField(controller: msg, maxLines: 5, decoration: const InputDecoration(labelText: 'Message', border: OutlineInputBorder())),
    gap,
    FilledButton.icon(onPressed: () => widget.onSms(selected?.phone ?? phone.text, msg.text), icon: const Icon(Icons.open_in_new), label: const Text('Open SMS')),
  ]);
}
...
class ContactPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext c) => Page(title: 'Contacts', fab: () => onEdit(null), children: [
    if (contacts.isEmpty) const Text('No contacts yet'),
    ...contacts.map((x) => Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Expanded(child: Text(x.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))), IconButton(onPressed: () => onFav(x), icon: Icon(x.fav ? Icons.star : Icons.star_border))]),
      Text(x.phone),
      if (x.note.isNotEmpty) Text(x.note),
...
class SchedulePage extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    ...
    return Page(title: 'Scheduler', fab: () => onEdit(null), children: [
      if (sorted.isEmpty) const Text('No scheduled texts'),
      ...sorted.map((j) => Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Expanded(child: Text(j.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18))), Chip(label: Text(j.status.name))]),
        Text(j.phone),
        Text('${_d(j.time)} ${_t(j.time)} ${j.due ? '• due' : ''}'),
        Text(j.text),
        Wrap(spacing: 8, children: [
          FilledButton.tonalIcon(onPressed: j.status == Status.scheduled ? () => onOpen(j) : null, icon: const Icon(Icons.sms), label: const Text('Open')),
...
class _ContactFormState extends State<ContactForm> {
  ...
  Widget build(BuildContext c) => Sheet(children: [
    ...
    TextField(controller: name, decoration: const InputDecoration(labelText: 'Name', border: OutlineInputBorder())), gap,
    TextField(controller: phone, decoration: const InputDecoration(labelText: 'Phone', border: OutlineInputBorder())), gap,
    TextField(controller: note, maxLines: 3, decoration: const InputDecoration(labelText: 'Note', border: OutlineInputBorder())), gap,
    FilledButton(onPressed: () => Navigator.pop(context, Contact(id: widget.c?.id ?? DateTime.now().microsecondsSinceEpoch.toString(), name: name.text, phone: phone.text, note: note.text, fav: widget.c?.fav ?? false)), child: const Text('Save'))
  ]);
}
...
class _JobFormState extends State<JobForm> {
  ...
  Widget build(BuildContext c) => Sheet(children: [
    ...
    DropdownButtonFormField<String>(initialValue: widget.contacts.any((x) => x.id == id) ? id : '', items: [const DropdownMenuItem(value: '', child: Text('Custom recipient')), ...widget.contacts.map((x) => DropdownMenuItem(value: x.id, child: Text('${x.name} • ${x.phone}')))], onChanged: (v) => setState(() => id = v ?? ''), decoration: const InputDecoration(labelText: 'Contact', border: OutlineInputBorder())), gap,
    if (id.isEmpty) ...[TextField(controller: name, decoration: const InputDecoration(labelText: 'Name', border: OutlineInputBorder())), gap, TextField(controller: phone, decoration: const InputDecoration(labelText: 'Phone', border: OutlineInputBorder())), gap],
    TextField(controller: msg, maxLines: 4, decoration: const InputDecoration(labelText: 'Message', border: OutlineInputBorder())), gap,
    Row(children: [Expanded(child: OutlinedButton(onPressed: pickDate, child: Text(_d(time)))), const SizedBox(width: 8), Expanded(child: OutlinedButton(onPressed: pickTime, child: Text(_t(time))))]), gap,
    FilledButton(onPressed: saveJob, child: const Text('Save'))
  ]);
  ...
  void saveJob() { final c = contact; Navigator.pop(context, Job(id: widget.job?.id ?? DateTime.now().microsecondsSinceEpoch.toString(), contactId: c?.id, name: c?.name ?? name.text, phone: c?.phone ?? phone.text, text: msg.text, time: time, status: Status.scheduled)); }
}
